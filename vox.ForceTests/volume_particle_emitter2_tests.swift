//
//  volume_particle_emitter2_tests.swift
//  vox.ForceTests
//
...
class volume_particle_emitter2_tests: XCTestCase {
    ...
    func testConstructors() throws {
        let sphere = SurfaceToImplicit2(surface: Sphere2(center: Vector2F(1.0, 2.0),
                                                         radiustransform: 3.0))
        
        let region = BoundingBox2F(point1: [0.0, 0.0], point2: [3.0, 3.0])
        ...
        XCTAssertEqual(Vector2F(), emitter.linearVelocity())
        XCTAssertEqual(0.0, emitter.angularVelocity())
        XCTAssertTrue(emitter.isEnabled())
    }
    
    func testEmit() {
        let sphere = SurfaceToImplicit2(surface: Sphere2(center: Vector2F(1.0, 2.0), radiustransform: 3.0))
        
        let box = BoundingBox2F(point1: [0.0, 0.0], point2: [3.0, 3.0])
        ...
        var pos:ArrayAccessor1<Vector2F> = particles.positions()
        ...
        for i in 0..<particles.numberOfParticles() {
            XCTAssertGreaterThan(3.0, length(pos[i] - Vector2F(1.0, 2.0)))
            XCTAssertTrue(box.contains(point: pos[i]))
            
            let r = pos[i]
            let w = 5.0 * Vector2F(-r.y, r.x)
            XCTAssertEqual(length(Vector2F(2.0, 4.5) + w - vel[i]), 0, accuracy: 1e-9)
        }
        
        emitter.setIsEnabled(enabled: false)
        frame.advance()
        emitter.setMaxNumberOfParticles(newMaxNumberOfParticles: 60)
        emitter.update(currentTimeInSeconds: frame.timeInSeconds(),
                       timeIntervalInSeconds: frame.timeIntervalInSeconds)
        ...
        XCTAssertEqual(55, particles.numberOfParticles())//modify
        
        pos = particles.positions()
        for i in 0..<particles.numberOfParticles() {
            pos[i] += Vector2F(2.0, 1.5)
        }
        
        frame.advance()
        emitter.update(currentTimeInSeconds: frame.timeInSeconds(),
                       timeIntervalInSeconds: frame.timeIntervalInSeconds)
        XCTAssertLessThan(51, particles.numberOfParticles())
    }
    
    func testBuilder() {
        let sphere = Sphere2(center: Vector2F(1.0, 2.0), radiustransform: 3.0)
        
        let emitter = VolumeParticleEmitter2.builder()
            .withSurface(surface: sphere)
            .withMaxRegion(bounds: BoundingBox2F(point1: [0.0, 0.0], point2: [3.0, 3.0]))
            .withSpacing(spacing: 0.1)
            .withInitialVelocity(initialVel: [-1.0, 0.5])
            .withLinearVelocity(linearVel: [3.0, 4.0])
            .withAngularVelocity(angularVel: 5.0)
        ...
        XCTAssertEqual(-1.0, emitter.initialVelocity().x)
        XCTAssertEqual(0.5, emitter.initialVelocity().y)
        XCTAssertEqual(Vector2F(3.0, 4.0), emitter.linearVelocity())
        XCTAssertEqual(5.0, emitter.angularVelocity())
    }
}
