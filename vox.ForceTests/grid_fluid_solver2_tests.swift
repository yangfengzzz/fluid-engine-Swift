//
//  grid_fluid_solver2_tests.swift
//  vox.ForceTests
//
...
class grid_fluid_solver2_tests: XCTestCase {
    ...
    func testResizeGridSystemData() {
        ...
        solver.resizeGrid(newSize: Size2(1, 2),
                          newGridSpacing: Vector2F(3.0, 4.0),
                          newGridOrigin: Vector2F(5.0, 6.0))
        
        XCTAssertEqual(1, solver.resolution().x)
        XCTAssertEqual(2, solver.resolution().y)
        XCTAssertEqual(3.0, solver.gridSpacing().x)
        XCTAssertEqual(4.0, solver.gridSpacing().y)
        XCTAssertEqual(5.0, solver.gridOrigin().x)
        XCTAssertEqual(6.0, solver.gridOrigin().y)
    }
    
    func testMinimumResolution() {
        let solver = GridFluidSolver2()
        
        solver.resizeGrid(newSize: Size2(1, 1),
                          newGridSpacing: Vector2F(1.0, 1.0),
                          newGridOrigin: Vector2F())
        solver.velocity().fill(value: Vector2F())
    ...
    func testGravityOnly() {
        ...
        solver.resizeGrid(newSize: Size2(3, 3),
                          newGridSpacing: Vector2F(1.0 / 3.0, 1.0 / 3.0),
                          newGridOrigin: Vector2F())
        solver.velocity().fill(value: Vector2F())
        ...
        solver.velocity().forEachUIndex(){(i:size_t, j:size_t) in
            XCTAssertEqual(0.0, solver.velocity().u(i: i, j: j), accuracy: 1e-8)
        }
        
        solver.velocity().forEachVIndex(){(i:size_t, j:size_t) in
            if (j == 0 || j == 3) {
                XCTAssertEqual(0.0, solver.velocity().v(i: i, j: j), accuracy: 1e-8)
            } else {
                XCTAssertEqual(-0.1, solver.velocity().v(i: i, j: j), accuracy: 1e-8)
            }
        }
    }
}
