...
class SphSystemData3: ParticleSystemData3 {
    ...
    /// (SphSystemData3::buildNeighborSearcher) before calling this function.
    func updateDensities() {
        let p:ArrayAccessor1<Vector3F> = positions()
    ...
    func sumOfKernelNearby(position origin:Vector3F)->Float {
        var sum:Float = 0.0
        ...
        neighborSearcher().forEachNearbyPoint(origin: origin, radius: _kernelRadius){
            (_:size_t, neighborPosition:Vector3F) in
    ...
    /// - Warning: You must update the neighbor searcher
    /// (SphSystemData3::buildNeighborSearcher) before calling this function.
    func interpolate(origin:Vector3F,
    ...
    /// (SphSystemData3::buildNeighborSearcher) before calling this function.
    func interpolate(origin:Vector3F,
                     values:ConstArrayAccessor1<Vector3F>)->Vector3F {
        var sum = Vector3F()
        let d:ArrayAccessor1<Float> = densities()
        let kernel = SphStdKernel3(kernelRadius: _kernelRadius)
        let m = mass()
    ...
    /// (SphSystemData3::buildNeighborLists) before calling this function.
    func gradientAt(i:size_t,
                    values:ConstArrayAccessor1<Float>)->Vector3F {
        var sum = Vector3F()
        let p:ArrayAccessor1<Vector3F> = positions()
        let d:ArrayAccessor1<Float> = densities()
    ...
    func computeMass() {
        ...
        let sampleBound = BoundingBox3F(
            point1: Vector3F(-1.5 * _kernelRadius, -1.5 * _kernelRadius,  -1.5 * _kernelRadius),
            point2: Vector3F(1.5 * _kernelRadius, 1.5 * _kernelRadius,  1.5 * _kernelRadius))
