...
#ifndef INCLUDE_VOX_LEVEL_SET_UTILS_METAL_
#define INCLUDE_VOX_LEVEL_SET_UTILS_METAL_
...
inline T smearedDeltaSdf(T phi) {
    if (fabs(phi) > 1.5) {
        ...
        return 1.0 / 3.0 + 1.0 / 3.0 * cos(pi<T>() * phi / 1.5);
    }
}
...
template <typename T>
void cycleArray(thread T* arr, int size) {
    T t = arr[0];
...
T distanceToZeroLevelSet(T phi0, T phi1) {
    if (fabs(phi0) + fabs(phi1) > kEpsilonF) {
        return fabs(phi0) / (fabs(phi0) + fabs(phi1));
    } else {
        return static_cast<T>(0.5);
    }
}

#endif  // INCLUDE_VOX_LEVEL_SET_UTILS_METAL_
