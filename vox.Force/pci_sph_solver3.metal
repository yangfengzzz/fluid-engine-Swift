...
namespace PciSphSolver3 {
    kernel void accumulatePressureForce1(device float *pressures [[buffer(0)]],
        ...
        pressures[id] = 0.0;
        _pressureForces[id] = float3();
        _densityErrors[id] = 0.0;
        ds[id] = densities[id];
    }
    
    kernel void accumulatePressureForce2(device float3 *_tempVelocities [[buffer(0)]],
                                         device float3 *velocities [[buffer(1)]],
                                         device float3 *forces [[buffer(2)]],
                                         device float3 *_pressureForces [[buffer(3)]],
                                         device float3 *_tempPositions [[buffer(4)]],
    ...
    kernel void accumulatePressureForce3(device int *_neighborLists_buffer [[buffer(0)]],
                                         ...
                                         device float3 *_tempPositions [[buffer(2)]],
                                         ...
                                         uint id [[thread_position_in_grid]],
                                         uint size [[threads_per_grid]]) {
        SphStdKernel3 Sphkernel(radius);
    ...
    kernel void accumulatePressureForce4(device float3 *forces [[buffer(0)]],
                                         device float3 *_pressureForces [[buffer(1)]],
                                         uint id [[thread_position_in_grid]],
                                         uint size [[threads_per_grid]]) {
        forces[id] += _pressureForces[id];
