//
//  vertex_centered_vector_grid3_impl.metal
//  vox.Force
//
...
float3 VertexCenteredVectorGrid3::dataOrigin() const {
    return origin();
}

float3 VertexCenteredVectorGrid3::dataPosition(size_t i, size_t j, size_t k) const {
    float3 o = dataOrigin();
    return o + gridSpacing() * float3(i, j, k);
}

const uint3 VertexCenteredVectorGrid3::resolution() const { return _resolution; }

const float3 VertexCenteredVectorGrid3::origin() const { return _origin; }

const float3 VertexCenteredVectorGrid3::gridSpacing() const { return _gridSpacing; }

float3 VertexCenteredVectorGrid3::cellCenterPosition(size_t i, size_t j, size_t k) const {
    float3 h = _gridSpacing;
    float3 o = _origin;
    return o + h * float3(i + 0.5, j + 0.5, k + 0.5);
}

const float3 VertexCenteredVectorGrid3::operator()(size_t i, size_t j, size_t k) const {
    return _const_accessor(i, j, k);
}

device float3& VertexCenteredVectorGrid3::operator()(size_t i, size_t j, size_t k) {
    return _accessor(i, j, k);
}

float VertexCenteredVectorGrid3::divergenceAtDataPoint(size_t i, size_t j, size_t k) const {
...
float3 VertexCenteredVectorGrid3::curlAtDataPoint(size_t i, size_t j, size_t k) const {
    const uint3 ds = _const_accessor.size();
    const float3 gs = gridSpacing();
...
float3 VertexCenteredVectorGrid3::sample(const float3 x) const { return _linearSampler(x); }

float VertexCenteredVectorGrid3::divergence(const float3 x) const {
...
float3 VertexCenteredVectorGrid3::curl(const float3 x) const {
    array<uint3, 8> indices;
    array<float, 8> weights;
    _linearSampler.getCoordinatesAndWeights(x, &indices, &weights);
...
VertexCenteredVectorGrid3::VectorDataAccessor VertexCenteredVectorGrid3::dataAccessor() {
    return _accessor;
}

VertexCenteredVectorGrid3::ConstVectorDataAccessor VertexCenteredVectorGrid3::constDataAccessor() const {
