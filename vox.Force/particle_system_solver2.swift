...
class ParticleSystemSolver2: PhysicsAnimation {
    var _dragCoefficient:Float = 1e-4
    var _restitutionCoefficient:Float = 0.0
    var _gravity = Vector2F(0.0, kGravity)
    
    var _particleSystemData:ParticleSystemData2
    var _newPositions = ParticleSystemData2.VectorData()
    var _newVelocities = ParticleSystemData2.VectorData()
    var _collider:Collider2?
    var _emitter:ParticleEmitter2?
    var _wind:VectorField2
    ...
    /// Returns the emitter.
    func emitter()->ParticleEmitter2? {
        return _emitter
    }
    
    /// Sets the emitter.
    func setEmitter(newEmitter:ParticleEmitter2) {
        _emitter = newEmitter
        newEmitter.setTarget(particles: _particleSystemData)
    }
    
    /// Returns the wind field.
    func wind()->VectorField2 {
        return _wind
    }
    ...
    func setWind(newWind:VectorField2) {
        _wind = newWind
    }
    ...
    func resolveCollision(newPositions: inout ArrayAccessor1<Vector2F>,
                          newVelocities: inout ArrayAccessor1<Vector2F>) {
        if (_collider != nil) {
    ...
    func setParticleSystemData(newParticles:ParticleSystemData2) {
        _particleSystemData = newParticles
    }
    
    func beginAdvanceTimeStep(timeStepInSeconds:Double) {
        // Clear forces
        var forces:ArrayAccessor1<Vector2F> = _particleSystemData.forces()
    ...
    func endAdvanceTimeStep(timeStepInSeconds:Double) {
        // Update data
        let n = _particleSystemData.numberOfParticles()
        var positions:ArrayAccessor1<Vector2F> = _particleSystemData.positions()
        var velocities:ArrayAccessor1<Vector2F> = _particleSystemData.velocities()
    ...
    func accumulateExternalForces() {
        let n = _particleSystemData.numberOfParticles()
        var forces:ArrayAccessor1<Vector2F> = _particleSystemData.forces()
        let velocities:ArrayAccessor1<Vector2F> = _particleSystemData.velocities()
        let positions:ArrayAccessor1<Vector2F> = _particleSystemData.positions()
    ...
    func timeIntegration(timeStepInSeconds:Double) {
        let n = _particleSystemData.numberOfParticles()
        let forces:ArrayAccessor1<Vector2F> = _particleSystemData.forces()
        let velocities:ArrayAccessor1<Vector2F> = _particleSystemData.velocities()
        let positions:ArrayAccessor1<Vector2F> = _particleSystemData.positions()
        let mass = _particleSystemData.mass()
    ...
    class Builder: ParticleSystemSolverBuilderBase2<Builder> {
        /// Builds ParticleSystemSolver2.
        func build()->ParticleSystemSolver2 {
            return ParticleSystemSolver2(radius: _radius, mass: _mass)
        }
    }
...
class ParticleSystemSolverBuilderBase2<DerivedBuilder> {
...
//MARK:- GPU Method
extension ParticleSystemSolver2 {
    func accumulateExternalForces_GPU() {
        let _wind_const = _wind as? ConstantVectorField2
        ...
        parallelFor(beginIndex: 0, endIndex: n,
                    name: "ParticleSystemSolver2::accumulateExternalForces") {
                        ...
                        encoder.setBytes(&_gravity, length: MemoryLayout<Vector2F>.stride, index: index+2)
        }
    }
    ...
    func timeIntegration_GPU(timeStepInSeconds:Double) {
        ...
        var mass = _particleSystemData.mass()
        parallelFor(beginIndex: 0, endIndex: n,
                    name: "ParticleSystemSolver2::timeIntegration") {
                        (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
    ...
    func endAdvanceTimeStep_GPU(timeStepInSeconds:Double) {
        let n = _particleSystemData.numberOfParticles()
        parallelFor(beginIndex: 0, endIndex: n,
                    name: "ParticleSystemSolver2::endAdvanceTimeStep") {
                        (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
                        index = _newVelocities.loadGPUBuffer(encoder: &encoder, index_begin: index)
                        index = _newPositions.loadGPUBuffer(encoder: &encoder, index_begin: index)
                        index = _particleSystemData.velocities(encoder: &encoder, index_begin: index)
