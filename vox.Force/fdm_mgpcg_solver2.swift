...
class FdmMgpcgSolver2: FdmMgSolver2 {
    struct Preconditioner : PrecondTypeProtocol {
        typealias BlasType = FdmBlas2
        
        var system:FdmMgLinearSystem2?
        var mgParams = MgParameters<FdmBlas2>()
        
        mutating func build(system system_:inout FdmMgLinearSystem2,
                            mgParams mgParams_:MgParameters<FdmBlas2>) {
            system = system_
            mgParams = mgParams_
        }
        
        func solve(b:FdmVector2, x:inout FdmVector2) {
            // Copy dimension
            var mgX = system!.x
            var mgB = system!.x
    ...
    override func solve(system:inout FdmMgLinearSystem2)->Bool {
