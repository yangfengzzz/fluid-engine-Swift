...
class ApicSolver2Renderable: Renderable {
    var solver = ApicSolver2()
    ...
    func RightWall() {
        ...
        // Build emitter
        let box = Box2.builder()
            .withLowerCorner(pt: [Float(1.0 - 1.0 / 64.0), 0.0])
            .withUpperCorner(pt: [1.0, 0.5])
            .build()
    ...
    func Circular() {
        // Build solver
        solver = ApicSolver2.builder()
            .withResolution(resolution: [40, 40])
            .withDomainSizeX(domainSizeX: 1.0)
            .build()
        ...
        for _ in 0..<4 * resX * resX {
            let pt = Vector2F(Float.random(in: 0...1), Float.random(in: 0...1))
    ...
    func CircularWithFriction() {
        ...
        let collider = RigidBodyCollider2.builder()
            ...
            .build()
        // Sticky boundary
        collider.setFrictionCoefficient(newFrictionCoeffient: 100.0)
    ...
    func exportStates(pos:inout Array1<Particle>) {
        ...
        for i in 0..<posAccessor.size() {
            ...
            pos[i].position.y = posAccessor[i].y*4-1
        }
    }
