...
class SphSolver3: ParticleSystemSolver3 {
    ...
    init() {
        ...
        setParticleSystemData(newParticles: SphSystemData3())
    ...
    func sphSystemData()->SphSystemData3 {
        return particleSystemData() as! SphSystemData3
    }
    ...
    override func numberOfSubTimeSteps(
        ...
        let f:ArrayAccessor1<Vector3F> = particles.forces()
    ...
    func accumulatePressureForce(positions:ConstArrayAccessor1<Vector3F>,
                                 densities:ConstArrayAccessor1<Float>,
                                 pressures:ConstArrayAccessor1<Float>,
                                 pressureForces: inout ArrayAccessor1<Vector3F>) {
        let particles = sphSystemData()
        let numberOfParticles = particles.numberOfParticles()
        
        let massSquared = Math.square(of: particles.mass())
        let kernel =  SphSpikyKernel3(kernelRadius: particles.kernelRadius())
    ...
    func computePseudoViscosity(timeStepInSeconds:Double) {
        let particles = sphSystemData()
        let numberOfParticles = particles.numberOfParticles()
        let x:ArrayAccessor1<Vector3F> = particles.positions()
        var v:ArrayAccessor1<Vector3F> = particles.velocities()
        let d:ArrayAccessor1<Float> = particles.densities()
        
        let mass = particles.mass()
        let kernel =  SphSpikyKernel3(kernelRadius: particles.kernelRadius())
        
        var smoothedVelocities = Array1<Vector3F>(size: numberOfParticles)
        
        parallelFor(beginIndex: 0, endIndex: numberOfParticles){(i:size_t) in
            var weightSum:Float = 0.0
            var smoothedVelocity = Vector3F()
    ...
    class Builder: SphSolverBuilderBase3<Builder> {
        /// Builds SphSolver3.
        func build()->SphSolver3 {
            return SphSolver3(targetDensity: _targetDensity,
...
class SphSolverBuilderBase3<DerivedBuilder> {
...
extension SphSolver3 {
    func computePressure_GPU() {
        ...
        parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                    name: "SphSolver3::computePressure") {
    ...
    func accumulatePressureForce_GPU() {
        ...
        parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                    name: "SphSolver3::accumulatePressureForce") {
                        (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
    ...
    func accumulateViscosityForce_GPU() {
        ...
        parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                    name: "SphSolver3::accumulateViscosityForce") {
                        (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
                        index = particles.positions(encoder: &encoder, index_begin: index)
    ...
    func computePseudoViscosity_GPU(timeStepInSeconds:Double) {
        ...
        parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                    name: "SphSolver3::computePseudoViscosity") {
                        (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
                        index = particles.positions(encoder: &encoder, index_begin: index)
                        index = particles.velocities(encoder: &encoder, index_begin: index)
                        index = particles.densities(encoder: &encoder, index_begin: index)
