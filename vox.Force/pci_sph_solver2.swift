...
let kDefaultTimeStepLimitScale:Float = 5.0
...
class PciSphSolver2: SphSolver2 {
    var _maxDensityErrorRatio:Float = 0.01
    var _maxNumberOfIterations:UInt = 5
    
    var _tempPositions = ParticleSystemData2.VectorData()
    var _tempVelocities = ParticleSystemData2.VectorData()
    var _pressureForces = ParticleSystemData2.VectorData()
    var _densityErrors = ParticleSystemData2.ScalarData()
    ...
    override func accumulatePressureForce(timeStepInSeconds timeIntervalInSeconds:Double) {
        if Renderer.arch == .GPU {
            ...
            let kernel = SphStdKernel2(kernelRadius: particles.kernelRadius())
            
            // Initialize buffers
            parallelFor(beginIndex: 0, endIndex: numberOfParticles){(i:size_t) in
                p[i] = 0.0
                _pressureForces[i] = Vector2F()
            ...
            for k in 0..<_maxNumberOfIterations {
                ...
                _pressureForces.set(value: Vector2F())
    ...
    func computeDelta(timeStepInSeconds:Double)->Float {
        ...
        let origin = Vector2F()
        var sampleBound = BoundingBox2F(point1: origin, point2: origin)
        ...
        let kernel = SphSpikyKernel2(kernelRadius: kernelRadius)
        ...
        for i in 0..<points.size() {
            ...
            if (distanceSquared < kernelRadius * kernelRadius) {
                ...
                let direction = (distance > 0.0) ? point / distance : Vector2F()
    ...
    func computeBeta(timeStepInSeconds:Double)->Float {
        let particles = sphSystemData()
        return 2.0 * Math.square(of: particles.mass() * Float(timeStepInSeconds)
...
extension PciSphSolver2 {
    func accumulatePressureForce_GPU(timeStepInSeconds timeIntervalInSeconds:Double) {
        ...
        parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                    name: "PciSphSolver2::accumulatePressureForce1") {
        ...
        for k in 0..<_maxNumberOfIterations {
            // Predict velocity and position
            parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                        name: "PciSphSolver2::accumulatePressureForce2") {
                            (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
            ...
            // Compute pressure from density error
            parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                        name: "PciSphSolver2::accumulatePressureForce3") {
                            (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
        ...
        // Accumulate pressure force
        parallelFor(beginIndex: 0, endIndex: numberOfParticles,
                    name: "PciSphSolver2::accumulatePressureForce4") {
                        (encoder:inout MTLComputeCommandEncoder, index:inout Int) in
                        index = particles.forces(encoder: &encoder, index_begin: index)
                        index = _pressureForces.loadGPUBuffer(encoder: &encoder, index_begin: index)
        }
