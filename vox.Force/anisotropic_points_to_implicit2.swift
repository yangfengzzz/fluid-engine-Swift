//
//  anisotropic_points_to_implicit2.swift
//  vox.Force
//
...
/// 32.1 (2013): 5.
class AnisotropicPointsToImplicit2: PointsToImplicit2 {
    ...
    func convert(points:ConstArrayAccessor1<Vector2F>,
        ...
        let meanNeighborSearcher = PointParallelHashGridSearcher2.builder().build()
        meanNeighborSearcher.build(points: points)
        
        logger.info("Built neighbor searcher.")
        
        let meanParticles = SphSystemData2()
        ...
        // Compute G and xMean
        var gs = Array<matrix_float2x2>(repeating: matrix_float2x2(1), count: points.size())
        var xMeans = Array1<Vector2F>(size: points.size())
        ...
        parallelFor(beginIndex: 0, endIndex: points.size()){(i:size_t) in
            ...
            let getXMean = {(_:size_t, xj:Vector2F) in
            ...
            if (numNeighbors < _minNumNeighbors) {
                ...
                // perfectly lined up.
                var cov = matrix_float2x2(h * h)
                wSum = 0.0
                let getCov = {(_:size_t, xj:Vector2F) in
                ...
                // SVD
                var u = matrix_float2x2(1)
                var v = Vector2F()
                var w = matrix_float2x2(1)
                ...
                let invSigma = matrix_float2x2(diagonal: 1.0 / v)
                
                // Compute G
                let scale = sqrt(v.x * v.y)  // area preservation
        ...
        let meanNeighborSearcher2 = PointSimpleListSearcher2()
        meanNeighborSearcher2.build(points: xMeans.constAccessor())
        
        // Compute SDF
        let temp = output.clone()
        temp.fill(){(x:Vector2F) in
            var sum:Float = 0.0
            meanNeighborSearcher2.forEachNearbyPoint(origin: x, radius: r){
...
// MARK:- Utility
func p(distance:Float)->Float {
    let distanceSquared = distance * distance
    
    if (distanceSquared >= 1.0) {
        return 0.0
    } else {
        let x = 1.0 - distanceSquared
        return x * x * x
    }
}

func wij(distance:Float, r:Float)->Float {
    if (distance < r) {
        return 1.0 - Math.cubic(of: distance / r)
    } else {
        return 0.0
    }
}

func vvt(v:Vector2F)->matrix_float2x2 {
    return matrix_float2x2(rows: [SIMD2<Float>(v.x * v.x, v.x * v.y),
                                  SIMD2<Float>(v.y * v.x, v.y * v.y)])
}

func w(r:Vector2F, g:matrix_float2x2, gDet:Float)->Float {
    let sigma = 4.0 / kPiF
