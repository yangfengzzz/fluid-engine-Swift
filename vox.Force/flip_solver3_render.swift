...
class FlipSolver3Renderable: Renderable {
    var solver = FlipSolver3()
    ...
    func RotatingTank() {
        // Build solver
        solver = FlipSolver3.builder()
            .withResolution(resolution: [32, 32, 32])
            .withDomainSizeX(domainSizeX: 1.0)
            .build()
        
        // Build emitter
        let box = Box3.builder()
            .withLowerCorner(pt: [0.25, 0.25, 0.25])
            .withUpperCorner(pt: [0.75, 0.50, 0.75])
            .build()
        ...
        // Build collider
        let tank = Box3.builder()
            .withLowerCorner(pt: [-0.25, -0.25, -0.25])
            .withUpperCorner(pt: [0.25, 0.25, 0.25])
            .withTranslation(translation: [0.5, 0.5, 0.5])
            .withOrientation(orientation: simd_quatf(ix: 0, iy: 0, iz: 1, r: 0))
            .withIsNormalFlipped(isNormalFlipped: true)
            .build()
        
        let collider = RigidBodyCollider3.builder()
            .withSurface(surface: tank)
            .withAngularVelocity(angularVelocity: [0, 0, 2])
            .build()
        
        collider.setOnBeginUpdateCallback(){(col:Collider3, t:Double, _:Double) in
            if (t < 1.0) {
                var surf = col.surface()
                surf.transform.setOrientation(orientation: simd_quatf(ix: 0, iy: 0, iz: 1, r: Float(2.0 * t)))
                col.setSurface(newSurface: surf)
                (col as! RigidBodyCollider3).angularVelocity = [0, 0, 2]
            } else {
                (col as! RigidBodyCollider3).angularVelocity = [0, 0, 0]
            }
        }
    ...
    func FlipMinimal() {
        solver = FlipSolver3.builder()
            .withResolution(resolution: [32, 64, 32])
            .withDomainSizeX(domainSizeX: 1.0)
            .build()
        
        let sphere = Sphere3.builder()
            .withCenter(center: [0.5, 1.0, 0.5])
            .withRadius(radius: 0.15)
            .build()
        
        let emitter = VolumeParticleEmitter3.builder()
            .withSurface(surface: sphere)
            .withSpacing(spacing: 0.5 / 64.0)
            .build()
        
        solver.setParticleEmitter(newEmitter: emitter)
        
        let anotherSphere = Sphere3.builder()
            .withCenter(center: [0.5, 0.5, 0.5])
            .withRadius(radius: 0.15)
            .build()
        
        let collider = RigidBodyCollider3.builder()
            .withSurface(surface: anotherSphere)
            .build()
